...
public class Cart {
    ...
    public void addItem(InventoryItem item, int qty) {
        products.merge(item.getProduct().sku(), qty, Integer::sum); //updates existing pdt qty / insert new pdt entry
    ...
    public void removeItem(InventoryItem item, int qty) {
        ...
        //e.g. 3 apples in cart, want to rm 3 / 5
        if (current <= qty) {   //qty in cart <= qty passed
            qty = current;
            products.remove(item.getProduct().sku());
            System.out.printf("Item [%s] removed from basket%n", item.getProduct().name());
        } else {
            //e.g. 5 apples in cart, want to put back 2 apples
    ...
    public void printSalesSlip(Map<String, InventoryItem> inventory) {
        ...
        for (var cartItem : products.entrySet()) {      //entrySet - sku=qty
